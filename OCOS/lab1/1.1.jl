...
using Plots
Setting=(markerstrokewidth=0,color = :blue,markersize=2,legend=false)
...
function zprint(x_A,F,_t,name,path)
#     display(plot(zprint1(x_A,F,_t,name),zprint2(x_A,F,_t,name),layout = (1,2)))
    if saveZad == "y" savefig(zprint2(x_A,F,_t,name), "$path.png")
    else
...
function Zad11()
    for (n,i) in enumerate(F)
        zprint(t -> d_digit(t), i, -2:2, "Дельта, F_$n", "dF$n") # 1
        for (jn, j) in enumerate(A_1) zprint(t -> x_const(j, t), i, -2:2, "Константа, $j, F_$n", "const$(jn)F$n") end # 2
        for (jn, j) in enumerate(A_2) zprint(t -> x_e(j..., t), i, -1:10, "Экспонента, $(join(j, ", ")), F_$n", "e$(jn)F$n") end # 3
        for (jn, j) in enumerate(A_3) zprint(t -> x_garm(j..., t), i, -5:5, "Гармонические колебания, $(join(j, ", ")), F_$n", "g$(jn)F$n") end # 4
        zprint(t -> x_poligarm(t), i, -5:5, "Полигармонические, F_$n", "pgF$n") # 5
        for (jn,(w,j)) in enumerate([[w_1, "прямоугольное"], [w_2, "Ханна"], [w_3, "Хэмминга"]]) zprint(t -> w(t), i, -5:2, "Окно $j, F_$n", "win$(jn)F$n") end # 6
    end
