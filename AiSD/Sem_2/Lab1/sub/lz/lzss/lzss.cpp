...
sstrtobb& lzss_ns::operator<<(sstrtobb& os, const node& n) {
    ...
    if (n.len < (node::size + 7) / 8 + 1) {
        ...
        size_t tmp = n.pos << (sizeof(size_t) * 8 - node::size);
...
#define BUFFER_SIZE window_buffer_size * num_byte * 10
void lzss_ns::lzss_2(std::istream& stream_in, std::ostream& stream_out, const size_t window_buffer_size) {
    node::size = 1;
    while (((1ULL << node::size) - 1) < window_buffer_size)
        node::size++;
    logger(log_ns::DEV_ONLY | log_ns::NORMAL_LVL)<<"buffer size is "<<window_buffer_size<<std::endl;
    stream_out.put(0);
    size_t buffer_size = window_buffer_size * num_byte;
    sstrtobb bbs_out;
    #ifndef NDEBUG
    ...
    #endif
    std::string buffer;
    std::string sub_buffer(BUFFER_SIZE, 0);
    // size_t read_bites = 0;
    size_t start = 0;
    while(stream_in) {
        stream_in.read(sub_buffer.data(), BUFFER_SIZE);
        sub_buffer.resize(stream_in.gcount());
        logger(log_ns::DEV_ONLY | log_ns::NORMAL_LVL) << "update buffer" << std::endl;
        buffer+=sub_buffer;
        logger(log_ns::DEV_ONLY | log_ns::NORMAL_LVL)<<"buffer is " << buffer<<std::endl;
        size_t i = start;
        for (; i < buffer.size() - (stream_in.eof() ? 0 : buffer_size);) {
            node n1;
            if (i >= buffer_size)
                start = i - buffer_size;
            std::string_view str_vi(buffer.data() + start, std::min(buffer_size, i));
            logger(log_ns::DEV_ONLY | log_ns::NORMAL_LVL)<<"str_vi is "<<str_vi<<std::endl;
            for (size_t j = i + num_byte; j != std::min(i + buffer_size, buffer.size()) && j - i <= str_vi.size(); j+=num_byte){
                logger(log_ns::DEV_ONLY | log_ns::NORMAL_LVL) << "search "<<std::string_view(buffer.data() + i, j - i)<<std::endl;
                size_t tmp_pos = str_vi.find(std::string_view(buffer.data() + i, j - i), (n1.pos ? str_vi.size() - n1.pos : 0));
                if (tmp_pos == str_vi.npos) {
                    logger(log_ns::DEV_ONLY | log_ns::HARD_LVL) <<"not found. last is "<<n1.pos<<' '<<n1.len<<std::endl;
                    break;
            ...
            if (n1.len > buffer_size) {
                logger(log_ns::DEV_ONLY)<<"ERROR! buffer len "<<n1.pos<<" > " <<buffer_size<<std::endl;
                throw "ERR";
            } else if (n1.pos > buffer_size) {
                logger(log_ns::DEV_ONLY)<<"ERROR! buffer pos "<<n1.pos<<" > " <<buffer_size<<std::endl;
                throw "ERR";
            }
            #endif
            bbs_out.try_write(stream_out);
        ...
        if (buffer.size() < buffer_size) {
            logger(log_ns::DEV_ONLY) << "end file"<<std::endl;
            break;
        }
        logger(log_ns::DEV_ONLY | log_ns::NORMAL_LVL) << "clearing buffer" << std::endl;
        if (buffer.size() > 2 * buffer_size) {
            start = i % (2 * buffer_size);
            buffer.erase(0, buffer.size() - 2 * buffer_size);
        } else start = i;
    }
    #ifndef NDEBUG
    logger(log_ns::DEV_ONLY | log_ns::HARD_LVL)<<"out buffer now: ";
    for (const char& c : bbs_out.get_data())
...
void lzss_ns::de_lzss_1(std::istream& stream_in, std::ostream& stream_out, const size_t window_buffer_size) {
    ...
    while(stream_in) {
        ...
        logger(log_ns::DEV_ONLY | log_ns::HARD_LVL).write(buffer.data(), buffer.size());
        logger(log_ns::DEV_ONLY | log_ns::HARD_LVL)<<" size "<<buffer.size()<<std::endl;
        ...
        logger(log_ns::DEV_ONLY | log_ns::HARD_LVL)<<std::endl;
#endif
        while (ssbb.get_len_in_bit() >= 1 + num_byte * 8) {
            node n;
...
#ifndef NDEBUG
                if (n.pos * num_byte > ss_buffer.size()) {
                    logger() << "ERROR! pos very big!" << n.pos * num_byte << " > " <<ss_buffer.size() << std::endl;
                    throw "ERR";
                }
                if (n.len * num_byte > n.pos * num_byte) {
                    logger() << "ERROR! len very big!" << n.len * num_byte << " > " <<n.pos * num_byte << std::endl;
                    throw "ERR";
                }
                logger(log_ns::DEV_ONLY | log_ns::NORMAL_LVL)<<"write 1 "<< str_vw <<std::endl;
#endif
...
bool lzss_ns::try_read_node(sstrtobb& ssbb_in, node& n) {
    size_t tmp = ssbb_in.get_len_in_bit();
    if (tmp >= 1 + node::size && ssbb_in.get_data().front() & 0x80 ||
        tmp >= 1 + num_byte * 8 && !(ssbb_in.get_data().front() & 0x80)) {
        ssbb_in >> n;
        return true;
    }
    return false;
...
void lzss(std::istream& stream_in, std::ostream& stream_out) {
    lzss_2(stream_in, stream_out, lz::get_window(sub_commands.front(), 1 << 26));
}
